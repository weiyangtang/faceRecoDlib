...
class VideoCamera(object):
    def __init__(self, pull_flow_url):
        # 通过opencv获取实时视频流
...
def video_feed():
    return Response(gen(VideoCamera(PULL_FLOW_URL)),
                    mimetype='multipart/x-mixed-replace; boundary=frame')


@app.route('/rtmp_face_reco')  # 这个地址返回视频流响应
def rtmp_face_reco():
    pull_flow_url = request.args.get('pullFlowUrl')
    print('拉流地址：', pull_flow_url)
    return Response(gen(VideoCamera(pull_flow_url)),
                    mimetype='multipart/x-mixed-replace; boundary=frame')
